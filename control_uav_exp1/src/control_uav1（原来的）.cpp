...
float Kp_cmd_xy=0.95;//0.95;//0.5还可以，0.3不行，因此最低设置为0.5
float Kp_cmd_z=2.5;//2.5//1.5
float Kp_cmd_vxvy=0.5;//0.09;//最低值：0.03
float Kp_cmd_vz=0.2;//0.2
float Ki_cmd_vxvy=0.04;//0.02//最低值：0.01
float Ki_cmd_vz=0.04;//0.04//0.15
float Kd_cmd_vxvy=0.01;//0.01;//最低值：
float Kd_cmd_vz=0.05;//0.08//0.2;//0.25

float Kp_pos_xy=0.95;//0.5;//
float Kp_pos_z=2.5;//1.8//1.5
float Kp_pos_vxvy=0.09;//0.065;//最低值：0.03
float Kp_pos_vz=0.2;//0.2
float Ki_pos_vxvy=0.02;//0.02//最低值：0.01
float Ki_pos_vz=0.04;//0.065//0.15
float Kd_pos_vxvy=0.01;//0.01;//最低值：
float Kd_pos_vz=0.08;//0.03//0.2;//0.25

float Kp_xy=0.95;//0.95;//0.5还可以，0.3不行，因此最低设置为0.5s
float Kp_z=2.5;//2.5//1.5
float Kp_vxvy=0.45;//0.09;//最低值：0.03
float Kp_vz=0.2;//0.2
float Ki_vxvy=0.04;//0.02//最低值：0.01
float Ki_vz=0.04;//0.04//0.15
float Kd_vxvy=0.01;//0.01;//最低值：
float Kd_vz=0.05;//0.08//0.2;//0.25
...
void calculte_attitude_send(nav_msgs::Odometry& odom_msg,control_uav_exp1::PositionCommand& cmd_msg,float dt,bool flag)
{
    ...
    std::cout<<"计算三轴速度误差: "<<" X: "<<vel_error[0]<<" Y: "<<vel_error[1]<<" Z: "<<vel_error[2]<<std::endl;
    ...
    bool stop_integral_Z = ( thrust_desired_Z  >= MPC_THR_MAX && vel_setpoint[2] >= 0.0f)||( thrust_desired_Z  <= MPC_THR_MIN && vel_setpoint[2] <= 0.0f);//0.9/0.1
    if (!stop_integral_Z) 
    {
        thurst_int[2] += Ki_vz  * vel_setpoint[2] * dt;
    ...
    vel_err_lim_x = vel_setpoint[0] - (thrust_desired_X - thrust_sp[0]) * arw_gain;
    vel_err_lim_y = vel_setpoint[1] - (thrust_desired_Y - thrust_sp[1]) * arw_gain;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber fcu_odom_sub = nh.subscribe("/mavros/local_position/odom", 50, FCU_odom_Callback);//30hz

    setpoint_raw_attitude_pub= nh.advertise<mavros_msgs::AttitudeTarget>("mavros/setpoint_raw/attitude", 10);//80hz

    uav_path_pub = nh.advertise<nav_msgs::Path>("/trajectory2", 1, true);
